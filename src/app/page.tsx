...
function Hero() {
  ...
  const { ref: liveSvgRef, y: liveY } = useParallax(0.012);
  ...
  return (
    <section className="relative min-h-screen flex flex-col bg-[#0d0d0d] overflow-hidden">
      ...
      {/* .LIVE SVG — absolute within hero, nearly static, very slow drift */}
      <div
        ref={liveSvgRef}
        className="absolute inset-0 flex items-center justify-center z-10 px-4 md:px-8 mix-blend-exclusion pointer-events-none will-change-transform"
        style={{ transform: `translateY(${liveY}px)` }}
      >
      ...
      {/* Content — lower quarter, offset 1/3 from left, blend-difference for SVG overlap */}
      <div
        ref={headRef}
...
/* ─── ABOUT + SERVICES wrapper (WHAT SVG spans both) ─── */
function AboutServices() {
  const aboutH2Ref = useFadeIn<HTMLDivElement>(0.15, "0px 0px -60px 0px", 0.065);
  const aboutBodyRef = useFadeIn<HTMLDivElement>(0.15, "0px 0px -60px 0px", 0.04);
  const servHeadRef = useFadeIn<HTMLDivElement>(0.15, "0px 0px -60px 0px", 0.06);
  const servBlocksRef = useFadeIn<HTMLDivElement>(0.1, "0px 0px -60px 0px", 0.035);
  const { ref: whatSvgRef, y: whatY } = useParallax(0.012);

  return (
    <section className="relative overflow-hidden">
      {/* WHAT SVG — absolute background, nearly static, spans About + Services */}
      <div
        ref={whatSvgRef}
        className="absolute inset-0 flex items-center justify-center pointer-events-none will-change-transform"
        style={{ zIndex: 0, transform: `translateY(${whatY}px)` }}
      >
      ...
      {/* ── SERVICES content ── */}
      <div id="services" className="relative z-10 py-24 md:py-32 lg:py-40">
        {/* Headline */}
        <div
          ref={servHeadRef}
          className="fade-up px-6 md:px-12 lg:px-16 max-w-[1920px] mx-auto mb-16 md:mb-20"
        ...
        <div className="relative z-10 px-6 md:px-12 lg:px-16 max-w-[1920px] mx-auto">
...
function Approach() {
  ...
  const { ref: howSvgRef, y: howY } = useParallax(0.01);

  return (
    <section id="approach" className="relative py-24 md:py-32 lg:py-40 overflow-hidden">
      {/* HOW SVG — absolute background, nearly static within approach section */}
      <div
        ref={howSvgRef}
        className="absolute inset-0 flex items-start justify-center pointer-events-none will-change-transform pt-[10%]"
        style={{ zIndex: 0, transform: `translateY(${howY}px)` }}
      >
      ...
      {/* Approach H2 — scrolls normally (different speed from SVG) */}
      <div className="relative z-10 px-6 md:px-12 lg:px-16 max-w-[1920px] mx-auto mb-16 md:mb-20">
        <div className="grid grid-cols-1 lg:grid-cols-4">
          <div className="lg:col-span-3">
            <h2 className="text-[32px] md:text-[42px] lg:text-[52px] font-semibold leading-[1.15] tracking-tight text-white uppercase">
              Approach
      ...
      <div
        ref={blocksRef}
        className="fade-up relative z-10 px-6 md:px-12 lg:px-16 max-w-[1920px] mx-auto mb-24 md:mb-32"
      >
        <div className="grid grid-cols-1 lg:grid-cols-3">
          <div className="hidden lg:block" />
          <div className="lg:col-span-2">
...
/* ─── WHEN (FINAL SECTION) ─── */
function When() {
  const { ref: whenSvgRef, y: whenY } = useParallax(0.01);

  return (
    <section id="pricing" className="relative py-16 md:py-24 lg:py-32 overflow-hidden">
      {/* WHEN SVG — absolute background, nearly static within this section */}
      <div
        ref={whenSvgRef}
        className="absolute inset-0 flex items-center justify-center pointer-events-none will-change-transform px-6 md:px-12 lg:px-16"
        style={{ zIndex: 0, transform: `translateY(${whenY}px)` }}
      >
        <Image
          src="/images/when.svg"
          alt=""
